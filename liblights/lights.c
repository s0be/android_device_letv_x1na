        ...
        struct light_state_t const* state)
{
    ...
    ALOGD("Setting backlight %i", brightness);
    if(!dev) {
        return -1;
    }
    ALOGD("XXX Setting backlight %i", brightness);
...
{
    int err = 0;
    ALOGD("Setting button backlight %i", state->color & 0xFF);
    if(!dev) {
        return -1;
    }
    ALOGD("XXX Setting button backlight %i", state->color & 0xFF);
    pthread_mutex_lock(&g_lock);
